...
#define VK_INSTANCE_LEVEL_FUNCTION(name)                                                                     \
	PFN_##name name = reinterpret_cast<PFN_##name>(glfwGetInstanceProcAddress(instance, #name));
...
DBG_EXPORT JS_METHOD(getInstanceProcAddress) {
	...
	THIS_VULKAN;
	REQ_STR_ARG(1, name)
	uint64_t addr = reinterpret_cast<uint64_t>(glfwGetInstanceProcAddress(instance, name.c_str()));
	RET_NUM(addr);
}
...
bool checkDeviceProperties(
	...
	vkGetPhysicalDeviceQueueFamilyProperties(
	    physical_device, &queue_families_count, &queue_family_properties[0]
	);
