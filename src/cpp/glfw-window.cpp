...
DBG_EXPORT void forEachWindow(const StatesIterator &fn) {
	std::for_each(states.begin(), states.end(), [&](WinState *state) {
		if (!state->window) {
			return;
		}
		fn(state);
...
// WAYLAND fix - will not be needed after 3.5? See: https://github.com/glfw/glfw/issues/2518
	...
	GLFWwindow *window = glfwCreateWindow(width, height, str.c_str(), monitor, isNoApi ? nullptr : _share);
...
DBG_EXPORT JS_METHOD(setWindowIcon) {
	...
	if (image.width <= 0 || image.height <= 0 || src == nullptr ||
